...
$userGroup = ArrayHelper::map(UserGroup::find()->all(), 'id', 'name');
?>
...
{error}'])->textInput(['maxlength' => true]) ?>
    ...
    </div>    <div class="col-sm-12 col-xs-12">    <?= $form->field($model, 'user_group_id', ['template' => '<div class="col-sm-3 text-right">{label}</div>
<div class="col-sm-9 col-xs-12">{input}</div>
{hint}
{error}'])->dropDownList($userGroup) ?>
