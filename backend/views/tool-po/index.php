...
/* @var $searchModel common\models\SearchToolPO */
...
[
    ...
    [
        ...
        'value' => function($model, $index, $column) {
            return ToolPO::getTPONo($model->purchase_order_no,$model->created) ;
        },
    ...
    [
        ...
        'label' => 'Total',
        'footer'=>SearchToolPO::pageTotal($dataProvider->models,'grand_total'),
    ],
    // 'clone',
    // 'created',
    // 'updated',
        ...
        'urlCreator' => function ($action, $model, $key, $index) {
            if ($action === 'preview') {
              //  $url ='?r=general-po/preview&id='.$model->id;
                $url = Url::to(['tool-po/preview','id'=>$model->id]);
                return $url;
            }
            if ($action === 'approve' && $model->approved != 'approved') {
                //$url ='?r=general-po/approve&id='.$model->id;
                $url = Url::to(['tool-po/approve', 'id'=>$model->id]);
                return $url;
            }
            if ($action === 'cancel') {
                //$url ='?r=general-po/cancel&id='.$model->id;
                $url = Url::to(['tool-po/cancel', 'id'=>$model->id]);
                return $url;
            }
            if ($action === 'delete') {
                //$url ='?r=general-po/delete-column&id='.$model->id;
                $url = Url::to(['tool-po/delete-column', 'id'=>$model->id]);
                return $url;
            }
        }
    ],
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="box">
                    ...
                    <br>
                    <?= Html::a('<i class="fa fa-plus"></i> New Tool PO', ['new'], ['class' => 'btn btn-default']) ?>
