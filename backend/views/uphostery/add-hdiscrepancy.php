    <div class="col-sm-12 col-xs-12">    
        <div class="form-group field-uphosterypreliminary-discrepancy">
            <div class="col-sm-3 text-right">
                <label class="control-label" for="uphosterypreliminary-discrepancy">Discrepancy</label>
            </div>
            <div class="col-sm-9 col-xs-12">
                <textarea id="uphosterypreliminary-discrepancy" class="form-control" name="UphosteryHiddenDamage[discrepancy][]" maxlength="2000" rows="4"></textarea>
                <div class="help-block">
                </div>
            </div>
        </div>
    </div> 


    <div class="col-sm-12 col-xs-12">    
        <div class="form-group field-uphosterypreliminary-corrective">
            <div class="col-sm-3 text-right">
                <label class="control-label" for="uphosterypreliminary-corrective">Corrective</label>
            </div>
            <div class="col-sm-9 col-xs-12">
                <textarea id="uphosterypreliminary-corrective" class="form-control" name="UphosteryHiddenDamage[corrective][]" maxlength="2000" rows="4"></textarea>
                <div class="help-block">
                </div>
            </div>

        </div>
    </div>  
<?php /*
    <div class="col-sm-12 col-xs-12">    
        <div class="form-group field-uphosterypreliminary-corrective">
            <div class="col-sm-3 text-right">
                <label class="control-label" for="uphosterypreliminary-corrective">Hidden Damage Attachment(s)</label>
            </div>
            <div class="col-sm-9 col-xs-12">
                <input type="file" name="UphosteryAttachment[attachment][hidden_damage][][]">
                <div class="help-block">
                </div>
            </div>

        </div>
    </div>  */ ?>
<script type="text/javascript"> confi(); </script>