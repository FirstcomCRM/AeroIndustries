...
class Stock extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['supplier_id', 'part_id', 'shelf_life','hour_used','receiver_no','storage_location_id','unit_id','sub_unit_id','currency_id'], 'integer'],
                ...
                if ($stockType == 'part') {
                    ...
                    if ( $model->quantity[$key] > 0 ) { 
                        // if ( $reusable == 0 ) {
                            ...
                            $eachStock->created = $currentDateTime;
                            $eachStock->save();
                        ...
                        // } else {
                            /* reusable */
                        //     for ( $loop = 0 ; $loop < $model->quantity[$key] ; $loop ++ ) {
                        //         $eachStock = new Stock();
                        //         $eachStock->receiver_no = $previousNo;
                        //         $eachStock->purchase_order_id = $id;
                        //         $eachStock->supplier_id = $model->supplier_id;
