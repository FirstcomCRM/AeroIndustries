...
class Generator extends \yii\gii\Generator
{
    ...
    public function generateActiveField($attribute)
    {
        ...
        if ($column->phpType === 'boolean') {
            return "\$form->field(\$model, '$attribute', ['template' => '<div class=\"col-sm-3 text-right\">{label}</div>\n<div class=\"col-sm-9 col-xs-12\">{input}</div>\n{hint}\n{error}'])->checkbox()";
            ...
            if (is_array($column->enumValues) && count($column->enumValues) > 0) {
                ...
                return "\$form->field(\$model, '$attribute', ['template' => '<div class=\"col-sm-3 text-right\">{label}</div>\n<div class=\"col-sm-9 col-xs-12\">{input}</div>\n{hint}\n{error}'])->dropDownList("
                    . preg_replace("/\n\s*/", ' ', VarDumper::export($dropDownOptions)).", ['class' => 'select2 form-control','prompt' => ''])";
            } elseif ($column->phpType !== 'string' || $column->size === null) {
                return "\$form->field(\$model, '$attribute', ['template' => '<div class=\"col-sm-3 text-right\">{label}</div>\n<div class=\"col-sm-9 col-xs-12\">{input}</div>\n{hint}\n{error}'])->$input()";
            } else {
                return "\$form->field(\$model, '$attribute', ['template' => '<div class=\"col-sm-3 text-right\">{label}</div>\n<div class=\"col-sm-9 col-xs-12\">{input}</div>\n{hint}\n{error}'])->$input(['maxlength' => true])";
            }
        }
    }
